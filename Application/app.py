...
class App(Tk):
    def __init__(self, androids):
        super().__init__()
        self.title('OSRS Bot')
        self.geometry(f"{236 * len(androids)}x500")
        #self.geometry(f"{516 * len(androids)}x709")
        ...
        for index, android in enumerate(androids):
            ...
            # Amount labels, shows amount of cooked food for android
            if android.script_number == 5:
                amount_label_info = Label(android_frame, text="Laps:", width=8)
            else:
                amount_label_info = Label(android_frame, text="Fished:", width=8)
            ...
            # Timer labels, shows time left until automatic logout for android
            time_label_info = Label(android_frame, text="Logout:", width=8)
            time_label_info.grid(column=0, row=3, pady=(10, 0), padx=(10, 10))
            time_label = Label(android_frame, text="0:00:00", width=8, relief=SUNKEN)
            time_label.grid(column=1, row=3, pady=(10, 0), padx=(10, 10))
            android.time_label = time_label

            # Stopwatch labels, shows runtime of script for android
            stopwatch_label_info = Label(android_frame, text="Runtime:", width=8)
            stopwatch_label_info.grid(column=0, row=4, pady=(10, 0), padx=(10, 10))
            stopwatch_label = Label(android_frame, text="0:00:00", width=8, relief=SUNKEN)
            stopwatch_label.grid(column=1, row=4, pady=(10, 0), padx=(10, 10))
            ...
            status_label_info.grid(column=0, row=5, pady=(10, 0), padx=(10, 10))
            status_label = Label(android_frame, text="Stopped", width=8, relief=SUNKEN)
            status_label.grid(column=1, row=5, pady=(10, 0), padx=(10, 10))
            ...
            # Login button
            login_button = Button(android_frame, text="Login", command=partial(self.login_button_function, index, android),
                                 bg="#5e5e5e", height=1, width=8, disabledforeground="#404040")
            login_button.grid(column=0, row=7, padx=(10, 10), pady=(10, 0), columnspan=1)
            login_button.bind('<Enter>', lambda event, high=True: toggle_button_highlight(event, high))
            login_button.bind('<Leave>', lambda event, high=False: toggle_button_highlight(event, high))
            login_button["state"] = DISABLED

            # Logout button
            logout_button = Button(android_frame, text="Logout", command=partial(self.logout_button_function, index, android),
                                bg="#BB86FC", height=1, width=8, disabledforeground="#404040")
            logout_button.grid(column=1, row=7, padx=(10, 10), pady=(10, 0), columnspan=1)
            logout_button.bind('<Enter>', lambda event, high=True: toggle_button_highlight(event, high))
            logout_button.bind('<Leave>', lambda event, high=False: toggle_button_highlight(event, high))
            ...
            self.login_buttons.append(login_button)
            self.logout_buttons.append(logout_button)
            self.image_labels.append(label)

        #for index, android in enumerate(androids):
            #thread = threading.Thread(
                #target=lambda: screenshot_loop(self, android, index), args=(), daemon=True)
            #thread.start()
    ...
    def add_options(self):
        ...
        # self.option_add("*Button.background", '#BB86FC')

    def start_button_function(self, index, android):
        self.disable_buttons(self.start_buttons[index], self.login_buttons[index], self.logout_buttons[index])
        self.enable_buttons(self.stop_buttons[index])
        android.script_stopped = False
        android.run_script()

    def stop_button_function(self, index, android):
        self.disable_buttons(self.stop_buttons[index])
    ...
    def login_button_function(self, index, android):
        self.disable_buttons(self.login_buttons[index])
        thread = threading.Thread(
            target=lambda: manual_login(self, index, android), args=(), daemon=True)
        thread.start()

    def logout_button_function(self, index, android):
        self.disable_buttons(self.start_buttons[index], self.logout_buttons[index])
        thread = threading.Thread(
            target=lambda: manual_logout(self, index, android), args=(), daemon=True)
        thread.start()
...
def stop_script(app, index, android):
    while True:
        if android.status_label["text"] == "Stopped":
            break
        sleep(0.1)
    app.enable_buttons(app.start_buttons[index], app.logout_buttons[index])
